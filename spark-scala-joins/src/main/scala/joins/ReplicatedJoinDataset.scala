...
object ReplicatedJoinDatasetMain {
  ...
  def main(args: Array[String]) {
    ...
    val conf = new SparkConf().setAppName("ReplicatedJoin-Dataset").setMaster("local[*]")
...
//    try { hdfs.delete(new org.apache.hadoop.fs.Path(args(1)), true) } catch { case _: Throwable => {} }
    ...
    val path2 = AtoBDataFrame.join(broadcast(BtoCDataFrame)).where(AtoBDataFrame("AtoB_2") === BtoCDataFrame("BtoC_1") && AtoBDataFrame("AtoB_1") =!= BtoCDataFrame("BtoC_2"))
    ...
    val triangles = path2.join(broadcast(AtoBDataFrame)).where(path2("BtoC_2") === AtoBDataFrame("start") && path2("AtoB_1") === AtoBDataFrame("end"))


    logger.info(triangles.queryExecution.executedPlan)
